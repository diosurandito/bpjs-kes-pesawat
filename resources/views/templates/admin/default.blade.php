        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    <div class="block-header" style="background-color: #284b8c;">
                            ...
                            <div class="col-lg-8 col-xl-12">
                                ...
                                <form method="post" id="edit_profile_form" enctype="multipart/form-data" autocomplete="off">
                                    ...
                                    <div class="block-content block-content-full text-right border-top">
                                        ...
                                        <button type="submit" class="btn btn-success" id="edit_profile_savebtn">Simpan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
  ...
  <script type="text/javascript">
    $(function () {
        ...
        $('#edit_profile_form').on('submit', function(e) {
            e.preventDefault();
            $('#edit_profile_savebtn').html('Mengirim..');
            $.ajax({
                ...
                success: function (data) {
                    if (data.failed_edit) {
                        $('#edit_profile_savebtn').html('Simpan');
                        Swal.fire({         //displays a pop up with sweetalert
                            icon: 'error',
                            title: data.failed_edit,
                            showConfirmButton: false,
                            timer: 5000
                        });
                    } else {
                        $('#current_username').val(data.new_username);
                        $('#edit_profile_savebtn').html('Simpan');
                        $('#edit_profile_form').trigger("reset");
                        $('#edit_profile_modal').modal('hide');
                        Swal.fire({         //displays a pop up with sweetalert
                            icon: 'success',
                            title: data.success_edit,
                            showConfirmButton: false,
                            timer: 3000
                        });
                    }
                ...
                error: function (data) {
                    ...
                    $('#edit_profile_savebtn').html('Save Changes');
                },
